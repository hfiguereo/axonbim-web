...
/** Axis length in XY and volume of the wall box. */
 ...
 * One derivation for plan and 3D.
 */
export function wallBoxMesh(wall: Wall): MeshBuffer {
  const dx = wall.p2.x - wall.p1.x;
  const dy = wall.p2.y - wall.p1.y;
  const length = Math.hypot(dx, dy);
  ...
  // Bottom rectangle (CCW from above): start-left, start-right, end-right, end-left
  const b0 = { x: wall.p1.x - nx * half, y: wall.p1.y - ny * half, z: z0 };
  const b1 = { x: wall.p1.x + nx * half, y: wall.p1.y + ny * half, z: z0 };
  const b2 = { x: wall.p2.x + nx * half, y: wall.p2.y + ny * half, z: z0 };
  const b3 = { x: wall.p2.x - nx * half, y: wall.p2.y - ny * half, z: z0 };
  const t0 = { ...b0, z: z1 };
  const t1 = { ...b1, z: z1 };
  const t2 = { ...b2, z: z1 };
  const t3 = { ...b3, z: z1 };
  ...
  const faces: { a: V; b: V; c: V; d: V; n: V }[] = [
    { a: b0, b: b1, c: b2, d: b3, n: { x: 0, y: 0, z: -1 } }, // bottom
    { a: t0, b: t3, c: t2, d: t1, n: { x: 0, y: 0, z: 1 } }, // top
    { a: b0, b: b3, c: t3, d: t0, n: { x: -nx, y: -ny, z: 0 } }, // left
    { a: b1, b: t1, c: t2, d: b2, n: { x: nx, y: ny, z: 0 } }, // right
    { a: b0, b: t0, c: t1, d: b1, n: { x: -ux, y: -uy, z: 0 } }, // start
    { a: b3, b: b2, c: t2, d: t3, n: { x: ux, y: uy, z: 0 } }, // end
  ];
